...
process cutadapt {
    ...
    cutadapt -j $task.cpus -a ${params.adapter} -A ${params.adapter} -m 12 -o ${name}.reads_1.trimmed.fastq.gz ${name}.reads_1.fastq.gz > ${name}.reads_1_cutadapt.log -p ${name}.reads_2.trimmed.fastq.gz --pair-filter=both
